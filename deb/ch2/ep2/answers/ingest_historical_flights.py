    ...
    def process(self, element, routes, *args, **kwargs):
        ...
        try:
            ...
            if route not in routes:
                ...
                yield beam.pvalue.TaggedOutput('rejects', element)  # send the record to rejects
                yield beam.pvalue.TaggedOutput('missing_routes', (route, 1))  # also send the route as missing
            ...
            logger.debug(f"input flight record is missing critical fields or of invalid type.")
...
def run():
    ...
    with beam.Pipeline(options=options) as p:
        ...
        # lookup routes
        rows, routes_rejects, missing_routes = (rows
                                                | beam.ParDo(BeamLookupRoute(), routes=routes).with_outputs('rejects', 'missing_routes', main='main'))
        ...
        output_routes = (missing_routes
                         | "gbr" >> beam.GroupByKey()   # calculate distinct missing routes
                         | "missing_routes_csv" >> beam.Map(lambda e: ','.join(list(e[0]))) # csv output the key (e[0] of key value tuple) which is (airline,src,dest)
                         | "missing_routes_out" >> beam.io.WriteToText(os.path.join(known_args.output, 'rejects/missing-routes'),
